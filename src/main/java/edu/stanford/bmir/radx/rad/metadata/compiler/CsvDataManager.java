...
public class CsvDataManager {
  ...
  private final String CONTRIBUTOR_ROLE_PATH_V1 = "/Data File Contributors/Contributor Role";
  private final String CONTRIBUTOR_ROLE_PATH_V2 = "/contributors/contributorRoles";
    ...
    for (Map.Entry<String, String> entry : csvData.entrySet()) {
      ...
      if (matcher.find()) {
        ...
        if(AttributeValueFieldUtil.isAttributeValue(templateSchemaArtifact, path)){
          ...
          if(isField(path)){
            processValues(value, path, null, hasId, isFirstVersionTemplate);
  ...
  private void processValues(String value, String path, Integer parentElementIndex, boolean hasId, boolean isFirstVersionTemplate){
    if(value != null && !value.isEmpty()){
      if(isFirstVersionTemplate && path.equals(CONTRIBUTOR_ROLE_PATH_V1)){
        var indexedPath = getIndexedPath(path, parentElementIndex, 0, hasId);
        groupedData.put(indexedPath, value);
  ...
  private void parseElementFieldValue(String csvField, String value, String csvAttribute, String mappedPath, Integer parentElementIndex, boolean hasId, boolean isFirstVersionTemplate){
    ...
    if(csvAttribute.startsWith(CREATOR_PREFIX) && !isFirstVersionTemplate){
      if(csvAttribute.startsWith(CREATOR_FULLNAME_PREFIX)){
        ...
        //add creator role
        processValues(CREATOR_ROLE, CONTRIBUTOR_ROLE_PATH_V2, parentElementIndex, hasId, isFirstVersionTemplate);
      } else{
        parentElementIndex += piNumber;
      }
    }

    processValues(value, mappedPath, parentElementIndex, hasId, isFirstVersionTemplate);
  }
